...
function saveAnnotations() {
    if (annotations != '') {
        ...
        save_name = `labeled_image112${index}`;
        console.log("Saved name", save_name);
        //downloadImage(save_name);
        console.log('Saved name', save_name);
        all_shapes.push(annotations);
        local_image = localStorage.getItem("recent-image");
        console.log("Loading local image");
        console.log(local_image);
        if(local_image)
        {
            document.querySelector("#local-image").setAttribute("src", local_image);
        ...
        save_name = "red4.jpg";
        var data = 'input_image=' + encodeURIComponent(save_name) +
               '&output_image=' + encodeURIComponent(save_name) +
               '&description=' + encodeURIComponent(annotations);
